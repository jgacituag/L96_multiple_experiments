...
base_nature_prefix = 'MultipleNature_Nature_Freq4_Den1.0_Type3'
nature_name = f'{base_nature_prefix}_{obs_err}'
gec = 'GEC'
conf.GeneralConf['NatureName'] = nature_name
conf.GeneralConf['ObsFile'] = f'/home/jorge.gacitua/experimentos/L96_multiple_experiments/data/Nature/Nature_JR{nature_name}.npz'
...
conf.DAConf['AlphaTempScale'] = 0            #Scale factor to obtain the tempering factors on each tempering iteration.
...
out_filename = f'/home/jorge.gacitua/experimentos/L96_multiple_experiments/data/LETKF/ORIG_LETKF_{nature_name}_Nens{nens}_NTemp{ntemp}_{gec}.npz'

print(f'\n=== Running experiment: {nature_name} with NTemp={ntemp} ===\n')

# Inicialización
#results = []
AlphaTempList=[np.array([1]) , np.array([90,1]) , np.array([90,5,1]) ]
...
for iinf, mult_inf in enumerate(mult_inf_range):
    for iloc, loc_scale in enumerate(loc_scale_range):
        ...
        conf.DAConf['AlphaTemp'] = AlphaTempList[ntemp - 1]
        conf.DAConf['AlphaTempScale'] = 0.0
