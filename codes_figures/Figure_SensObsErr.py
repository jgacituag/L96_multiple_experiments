...
base_path  = '/home/jruiz/salidas/DABA/Lorenz_96/experiments/npz/'
...
for i, obs in enumerate(obs_errs):
    for j, ntemp in enumerate(ntemps):
        #filename = f'LETKF_Paper_Nature_Freq{freq}_Den{den}_Type3_ObsErr{obs}_NTemp{ntemp}_alpha{alpha}{gec}.npz'
        filename = f'Sensitivity_experiment_multinfyloc_LETKF_ptemp{alpha}_MultipleNature_Nature_Freq{freq}_Den{den}_Type3_ObsErr{obs}.npz'
        ...
        if os.path.exists(filepath):
            ...
            total_analysis_rmse = data['total_analysis_rmse'][:,:,j]
...
for i, obs in enumerate(obs_errs):
    axes[i, 0].annotate(rf"$\bf{{\sigma_0 = {obs}}}$", xy=(-0.15, 0.5), xycoords='axes fraction',
...
figname = f"Figure_AnalysisRMSE_Freq{freq}_Den{den}_Type3_ObsErr{obs}_NTemp{ntemp}_JR"
