...
def plot_best_params(base_path, output_dir, Freq, Den, ObsErr, suffix, percentual=True):
    ...
    Loads Best_params_*.npz and produces:
      ...
      - Two bar charts with RMSE means (over alpha, over Nens)
    ...
    rmse = data["rmse_values"]            # shape (n_nens, n_alpha, 3)
    ...
    if rmse.ndim != 3 or rmse.shape[2] < 3:
        raise ValueError("rmse_values must have shape (n_nens, n_alpha, 3).")

    # Extract methods
    letkf    = rmse[:, :, 0]  # (nens, alpha)
    letkf_t2 = rmse[:, :, 1]
    letkf_t3 = rmse[:, :, 2]
    ...
    mean_rmse_letkf    = np.nanmean(letkf, axis=1)     # mean over alpha
    mean_rmse_letkf_t2 = np.nanmean(letkf_t2, axis=1)  # mean over alpha
    mean_rmse_letkf_t3 = np.nanmean(letkf_t3, axis=1)  # mean over alpha
    x = np.arange(len(nens_labels)) * 4
    axes[2].bar(x,     mean_rmse_letkf,    color='gray',  label='LETKF')
    axes[2].bar(x + 1, mean_rmse_letkf_t2, color='Teal',  label='LETKF-T2')
    axes[2].bar(x + 2, mean_rmse_letkf_t3, color='Maroon',label='LETKF-T3')
    ...
    mean_rmse_letkf    = np.nanmean(letkf, axis=0)     # mean over Nens
    mean_rmse_letkf_t2 = np.nanmean(letkf_t2, axis=0)  # mean over Nens
    mean_rmse_letkf_t3 = np.nanmean(letkf_t3, axis=0)  # mean over Nens
    x = np.arange(len(alpha_vals)) * 4
    axes[3].bar(x,     mean_rmse_letkf,    color='gray',  label='LETKF')
    axes[3].bar(x + 1, mean_rmse_letkf_t2, color='Teal',  label='LETKF-T2')
    axes[3].bar(x + 2, mean_rmse_letkf_t3, color='Maroon',label='LETKF-T3')
...
def plot_best_params_heatmaps(base_path, output_dir, Freq, Den, ObsErr, suffix, percentual=True):
    ...
    data = np.load(npz_path, allow_pickle=True)
    rmse         = data["rmse_values"]      # (n_nens, n_alpha, 3)
...
def plot_best_params_bars(base_path, output_dir, Freq, Den, ObsErr, suffix, ylim=(0, 3.6)):
    ...
    Create ONLY the two bar charts:
      - Mean RMSE over alpha (per Nens)
      - Mean RMSE over Nens (per alpha)
...
if __name__ == "__main__":
    ...
    if work_place == 'hydra':
        ...
        output_dir = r'/home/jorge.gacitua/salidas/L96_multiple_experiments/figures_Prespinup/Best_params_RMSE'
    elif work_place == 'ubuntu':
        base_path = r'/media/jgacitua/storage/L96_multiple_experiments/data/summary'
        output_dir = r'/media/jgacitua/storage/L96_multiple_experiments/data/figures_Prespinup/Best_params_RMSE'
    elif work_place == 'win':
        base_path  = r'C:\Users\jyndd\OneDrive\Doctorado\Experimentos\L96_multiple_experiments\data\summary'
        output_dir = r'C:\Users\jyndd\OneDrive\Doctorado\Experimentos\L96_multiple_experiments\figures_Prespinup\Best_params_RMSE'
