...
def plot_best_params(base_path, output_dir, Freq, Den, ObsErr, suffix, percentual=True):
    ...
    Loads Best_params_*.npz and produces:
      ...
      - Two bar charts with ratio means (over alpha, over Nens)
    ...
    ratio = data["ratio_values"]            # shape (n_nens, n_alpha, 3)
    ...
    if ratio.ndim != 3 or ratio.shape[2] < 3:
        raise ValueError("ratio_values must have shape (n_nens, n_alpha, 3).")

    # Extract methods
    letkf    = ratio[:, :, 0]  # (nens, alpha)
    letkf_t2 = ratio[:, :, 1]
    letkf_t3 = ratio[:, :, 2]
    ...
    mean_ratio_letkf    = np.nanmean(letkf, axis=1)     # mean over alpha
    mean_ratio_letkf_t2 = np.nanmean(letkf_t2, axis=1)  # mean over alpha
    mean_ratio_letkf_t3 = np.nanmean(letkf_t3, axis=1)  # mean over alpha
    x = np.arange(len(nens_labels)) * 4
    axes[2].bar(x,     mean_ratio_letkf,    color='gray',  label='LETKF')
    axes[2].bar(x + 1, mean_ratio_letkf_t2, color='Teal',  label='LETKF-T2')
    axes[2].bar(x + 2, mean_ratio_letkf_t3, color='Maroon',label='LETKF-T3')
    ...
    mean_ratio_letkf    = np.nanmean(letkf, axis=0)     # mean over Nens
    mean_ratio_letkf_t2 = np.nanmean(letkf_t2, axis=0)  # mean over Nens
    mean_ratio_letkf_t3 = np.nanmean(letkf_t3, axis=0)  # mean over Nens
    x = np.arange(len(alpha_vals)) * 4
    axes[3].bar(x,     mean_ratio_letkf,    color='gray',  label='LETKF')
    axes[3].bar(x + 1, mean_ratio_letkf_t2, color='Teal',  label='LETKF-T2')
    axes[3].bar(x + 2, mean_ratio_letkf_t3, color='Maroon',label='LETKF-T3')
    axes[3].set_xticks(x + 1)
    axes[3].set_xticklabels(alpha_ticklabels)
    axes[3].set_ylabel("Ratio", fontsize=14)
...
def plot_best_params_heatmaps(base_path, output_dir, Freq, Den, ObsErr, suffix, percentual=True):
    ...
    data = np.load(npz_path, allow_pickle=True)
    ratio         = data["ratio_values"]      # (n_nens, n_alpha, 3)
...
def plot_best_params_bars(base_path, output_dir, Freq, Den, ObsErr, suffix, ylim=(0, 3.6)):
    ...
    Create ONLY the two bar charts:
      - Mean ratio over alpha (per Nens)
      - Mean ratio over Nens (per alpha)
...
if __name__ == "__main__":
    ...
    if work_place == 'hydra':
        ...
        output_dir = r'/home/jorge.gacitua/salidas/L96_multiple_experiments/figures_Prespinup/Best_params_ratio'
    elif work_place == 'ubuntu':
        base_path = r'/media/jgacitua/storage/L96_multiple_experiments/data/summary'
        output_dir = r'/media/jgacitua/storage/L96_multiple_experiments/data/figures_Prespinup/Best_params_ratio'
    elif work_place == 'win':
        base_path  = r'C:\Users\jyndd\OneDrive\Doctorado\Experimentos\L96_multiple_experiments\data\summary'
        output_dir = r'C:\Users\jyndd\OneDrive\Doctorado\Experimentos\L96_multiple_experiments\figures_Prespinup\Best_params_ratio'
